...
def actualizar_registro_cliente(id_cliente, cliente, id_existing_record, dias_credito_promedio, id_existing_records, supabase_table):
    ...
    if id_existing_record != 0:
        ...
        responseCreate = supabase_client_out.table(supabase_table).insert({"dias_promedio": dias_credito_promedio, "id_analisis": condition, "cliente": cliente}).execute()
        # Verifica la respuesta exitosa y maneja errores si es necesario
        ...
        if (input_response.status_code == 200) & (input_response_clientes.status_code == 200):
            ...
            for cliente in lista_cliente:
                ...
                if responseClienteDF.isEmpty() == False:
                    ...
                    print(f"Dias crédito promedio: {dias_credito_promedio}")
                    # <<< INSERCIÓN >>>
