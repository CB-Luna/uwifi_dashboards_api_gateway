...
spark = SparkSession.builder.appName("GraficaTotalFacturadoCondicionPago").getOrCreate()
...
def actualizar_registro_todos(id_existing_record, key, value, id_existing_records, supabase_table):
    ...
    if id_existing_record != 0:
        ...
        responseCreate = supabase_client_out.table(supabase_table).insert({"dias":key, "total_facturado": value, "id_analisis": condition, "cliente": "Todos"}).execute()
        # Verifica la respuesta exitosa y maneja errores si es necesario
